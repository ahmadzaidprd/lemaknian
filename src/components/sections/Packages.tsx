    ...
    <section id="paket" style={{ padding: "120px 28px", position: "relative", overflow: "hidden" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "flex-end", gap: 24, marginBottom: 56 }}>
          ...
          <Reveal delay={300}>
            <a href="#menu" className="btn-secondary" style={{ textDecoration: "none" }}>Lihat semua paket →</a>
          </Reveal>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 20 }}>
          {paketData.map((p, i) => (
            <Reveal key={p.id} delay={i * 100}>
              <TiltCard className="card" max={8} style={{
                padding: 28, height: "100%", minHeight: 460, position: "relative",
                borderColor: p.popular ? "var(--accent)" : undefined,
                borderWidth: p.popular ? 2 : 1,
                background: p.popular ? "linear-gradient(165deg, rgba(var(--accent-rgb),0.08), rgba(var(--accent-rgb),0.0))" : undefined,
              }}>
                <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
                  ...
                  <div style={{ color: "var(--text-primary)", fontSize: 22, fontWeight: 500, marginBottom: 6, fontFamily: "Fraunces, serif", letterSpacing: "-0.01em" }}>{p.nama}</div>
                  ...
                  <div style={{ color: "var(--text-secondary)", fontSize: 13, lineHeight: 1.55, marginBottom: 18, flex: 1 }}>{p.deskripsi}</div>
